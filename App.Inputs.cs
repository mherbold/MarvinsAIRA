				...
				try
				{
					...
					if ( joystickUpdateArray.Length > 0 )
					{
						if ( Input_AnyPressedButton.DeviceInstanceGuid == Guid.Empty )
						{
							...
							{
								if ( ( joystickUpdate.Offset >= JoystickOffset.Buttons0 ) & ( joystickUpdate.Offset <= JoystickOffset.Buttons127 ) || (joystickUpdate.Offset >= JoystickOffset.PointOfViewControllers0) & (joystickUpdate.Offset <= JoystickOffset.PointOfViewControllers3) )
								{
